...
function CemberinAlani(yariCap, piSayisi) {
  const yariCapKaresi = Math.pow(yariCap, 2);
  return piSayisi * yariCapKaresi;
}
console.log("Çemberin Alanı :", CemberinAlani(5, pi));
...
console.log("Üçe Tam Bölünenler Dizisi :", ucetambolunenler);

//3c çözümü:

const ucebolunenlerintoplami = ucetambolunenler.reduce((toplam, baslagıc) => {
  return toplam + baslagıc;
});
...
console.log("500'den küçük ve sıralı sayılar :", siralisayilar);
...
for (const sayi in tekrarSayilari) {
  if (tekrarSayilari[sayi] > 1) {
    const tekrarString = `${sayi} sayısı ${tekrarSayilari[sayi]} kere tekrar edilmiştir`;
